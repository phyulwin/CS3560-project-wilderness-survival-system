...
# --- Custom Dialog for Dropdown Menus ---
...
class Terrain(abc.ABC):
    def __init__(self, m, w, f, c):
        self.movement_cost = m
        self.water_cost = w
        self.food_cost = f
        self.color = c
...
class Item(abc.ABC):
    def __init__(self, is_repeating, symbol):
        ...
        self.symbol = symbol
    @abc.abstractmethod
    def on_collect(self, player, gui): pass
...
class Map:
    def __init__(self, w, h, d):
        ...
        self.height = h
        self.difficulty = d
        self.squares = [[None for _ in range(w)] for _ in range(h)]
        self.populate_map()
    ...
    def populate_map(self):
        ...
        for r in range(self.height):
            for c in range(self.width):
                self.squares[r][c] = Square(random.choices(types, weights=weights, k=1)[0]())
                if random.random() < chance:
                    item_type = random.choice([FoodBonus, WaterBonus, GoldBonus, Trader])
                    if item_type == Trader and random.random() > .2: continue
...
# --- Player ---
class Player:
    def __init__(self, vc, bc, map_w, map_h):
        sf = 15 + (map_w * .75)
    ...
    def can_move(self, d, m):
        nr = self.row + d[0]
        nc = self.col + d[1]
        ts = m.get_square(nr, nc)
        if not ts: return False
        cost = {'strength': ts.terrain.movement_cost, 'water': ts.terrain.water_cost, 'food': ts.terrain.food_cost}
        return self.current_strength >= cost['strength'] and self.current_water >= cost['water'] and self.current_food >= cost['food']

    def move(self, d, m):
        nr = self.row + d[0]
        nc = self.col + d[1]
        ts = m.get_square(nr, nc)
        if self.can_move(d, m):
            cost = {'strength': ts.terrain.movement_cost, 'water': ts.terrain.water_cost, 'food': ts.terrain.food_cost}
            self.row = nr
            self.col = nc
    ...
    def is_dead(self):
        threshold = 0.1
        return self.current_strength <= threshold or self.current_water <= threshold or self.current_food <= threshold

    def has_won(self, m):
        return self.col == m.width - 1

    def is_stuck(self, game_map):
        can_make_a_move = any(self.can_move(d, game_map) for d in Direction.ALL)
        if can_make_a_move: return False
        if self.current_water <= 0.5 or self.current_food <= 0.5: return True
        return False
...
class Vision(abc.ABC):
    ...
    def _find_closest_item(self, map_instance, player, item_class):
        ...
        visited = set([(player.row, player.col)])
        while q:
            r, c, path = q.popleft()
            sq = map_instance.get_square(r, c)
            if any(isinstance(i, item_class) for i in sq.items) and path: return Path(path, {})
...
class FarSightVision(Vision):
    ...
    def _find_closest_item(self, map_instance, player, item_class):
        ...
        while q:
            r, c, path = q.popleft()
            if any(isinstance(i, item_class) for i in map_instance.get_square(r,c).items) and path: return Path(path, {})
            for d in Direction.ALL:
                nr, nc = r + d[0], c + d[1]
...
class WSS_GUI(tk.Tk):
    def __init__(self):
        ...
        self.cell_size = 30
        self.setup_ui()
        self.after(100, self.setup_new_game)

    def setup_ui(self):
        mf = tk.Frame(self, bg="#34495e")
        mf.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        self.canvas = tk.Canvas(mf, bg="#ecf0f1", highlightthickness=0)
        self.canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5)
        sb = tk.Frame(mf, width=300, bg="#34495e")
        sb.pack(side=tk.RIGHT, fill=tk.Y, padx=5)
        sb.pack_propagate(False)

        tf = font.Font(family="Helvetica", size=16, weight="bold")
        lf = font.Font(family="Helvetica", size=11)

        tk.Label(sb, text="Player Stats", font=tf, fg="white", bg="#34495e").pack(pady=10)
        ...
        for n,v in self.stats_vars.items():
            f=tk.Frame(sb,bg="#34495e"); f.pack(fill=tk.X,pady=4)
        ...
        cf = tk.Frame(sb, bg="#34495e"); cf.pack(pady=20)
        self.next_turn_button = tk.Button(cf, text="Next Turn", command=self.run_turn, font=lf)
        self.next_turn_button.pack(pady=5)
        tk.Button(cf, text="New Game", command=self.setup_new_game, font=lf).pack(pady=5)

        tk.Label(sb, text="Game Log", font=tf, fg="white", bg="#34495e").pack(pady=10)
        self.log_text = tk.Text(sb, height=15, state=tk.DISABLED, bg="#2c3e50", fg="white", font=("Courier",9))
    ...
    def setup_new_game(self):
        ...
        difficulty = CustomDropdownDialog(self,"Difficulty","Choose difficulty:",["Easy","Medium","Hard"]).result
        ...
        vc = vision_map.get(vision_str)
        bc = brain_map.get(brain_str)
        w = simpledialog.askinteger("Map Size","Enter map width:",initialvalue=25,minvalue=10,maxvalue=50,parent=self)
        if not w: return
        h = simpledialog.askinteger("Map Size","Enter map height:",initialvalue=15,minvalue=10,maxvalue=50,parent=self)
        if not h: return
        ...
        self.log_message(f"New game: {w}x{h} ({difficulty})")
        self.log_message(f"Player: {vc.__name__}, {bc.__name__}")
    ...
    def draw_map(self):
        ...
        for r in range(self.game_map.height):
            for c in range(self.game_map.width):
                ...
                if sq.items:
                    self.canvas.create_text(x1+self.cell_size/2, y1+self.cell_size/2, text=sq.items[0].symbol, font=("Helvetica",12,"bold"), fill="black")
        if self.player:
    ...
    def check_game_over(self):
        ...
        elif self.player.has_won(self.game_map):
            self.log_message("VICTORY")
            messagebox.showinfo("Victory","You survived!")
            self.next_turn_button.config(state=tk.DISABLED)
        elif self.player.is_stuck(self.game_map):
            self.log_message("STALEMATE")
            messagebox.showinfo("Game Over", "You are stuck and cannot move!")
            self.next_turn_button.config(state=tk.DISABLED)
